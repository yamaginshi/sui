...
fn test_deterministic_addresses_by_domain() {
    ...
    let sui_address_0 = seed.new_deterministic_address::<DefaultAccountKeyPair>(&TEST_ID, Some(&domain_0));
    assert!(sui_address_0.is_ok());

    let sui_address_1 = seed.new_deterministic_address::<DefaultAccountKeyPair>(&TEST_ID, Some(&domain_1));
    assert!(sui_address_1.is_ok());
